...
# Функция для сохранения данных в JSON файле в папке пользователя
...
def submit():
    if entry_name.get() and entry_age.get() and entry_height.get() and entry_weight.get():
        ...
        profile_button = customtkinter.CTkButton(master=profile_list, text=entry_name.get(), font=customtkinter.CTkFont(family="Roboto", weight="bold" , size=22), width=500, height=50, command=choose_user)
...
entry_name = customtkinter.CTkEntry(master=enter, placeholder_text="Имя", state="normal")
entry_name.pack(pady=5, padx=10)

entry_age = customtkinter.CTkEntry(master=enter, placeholder_text="Возраст", state="normal")
entry_age.pack(pady=5, padx=10)

entry_height = customtkinter.CTkEntry(master=enter, placeholder_text="Рост", state="normal")
entry_height.pack(pady=5, padx=10)

entry_weight = customtkinter.CTkEntry(master=enter, placeholder_text="Вес", state="normal")
entry_weight.pack(pady=5, padx=10)

submit = customtkinter.CTkButton(master=enter, text="Создать", command=submit)
...
for profile in user_profiles:
    ...
    profile_button = customtkinter.CTkButton(master=profile_list, text=profile['username'], font=customtkinter.CTkFont(family="Roboto", weight="bold" , size=22), width=500, height=50, command=choose_user)
    profile_button.pack(pady=5)
